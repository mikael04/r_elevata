...
prop_ate_1ano_ant <- p_ij_n_ij_v_ij_pp_n %>%
  ...
  dplyr::select(proposta_id, proposta_data_cadastro, proposta_status, vendedor_nome, pp_valor_tot, negocio_tipo_negocio)
...
prop_ate_1ano_ant <- prop_ate_1ano_ant %>%
  ...
  dplyr::select(-proposta_id, vendedor_nome, valor_proposta) %>%
  dplyr::rename(Vendedor = vendedor_nome, Valor = valor_proposta) %>%
  ...
  dplyr::select(Vendedor, Valor, Status, 'Data de Cadastro')
...
tabela2 <- prop_ate_1ano_ant %>%
  DT::datatable(rownames = F,
                ...
                filter = 'top',
                extensions = list('Responsive'))

tabela3 <- prop_ate_1ano_ant %>%
  DT::datatable(rownames = F,
                escape = F,
                filter = 'top',
                extensions = 'Buttons',
...
table_reac <- reactable(prop_ate_1ano_ant, details = function(index) {
  htmltools::div(
    "Detalhes: ", index,
...
table_reac_2 <- reactable(prop_ate_1ano_ant, filterable = T, details = colDef(
  name = "More",
  details = JS("function(rowInfo) {
    return 'Detalhes da linha: ' + rowInfo.index +
...
htmlTable::htmlTable(prop_ate_1ano_ant)


library(tableHTML)


tableHTML::tableHTML(prop_ate_1ano_ant)
```
