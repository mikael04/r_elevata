...
prop_ate_1ano_ant <- prop_ate_1ano_ant %>%
  dplyr::rename(Proposta = proposta_id, Vendedor = vendedor_nome, Valor = valor_proposta)


prop_ate_1ano_ant <- prop_ate_1ano_ant %>%
  dplyr::mutate('Tipo de Negócio' = dplyr::if_else(negocio_tipo_negocio == "N", "Novo", "Usado")) %>%
  ...
  dplyr::select(Proposta, 'Data de Cadastro', Status, Vendedor, Valor, 'Tipo de Negócio')

#'<a href="http://rstudio.com">RStudio</a>'
prop_ate_1ano_ant <- prop_ate_1ano_ant %>%
  dplyr::mutate(Link = paste0('<a href="', paste0("https://letmegooglethat.com/?q=", Proposta),'", target=\"_blank\">Link da proposta</a>'))
...
tabela2 <- prop_ate_1ano_ant %>%
  DT::datatable(rownames = F,
                escape = F,
                options = list(columnDefs = list(list(className = 'dt-center', targets = c(0,1,2,3,4,5,6))),
